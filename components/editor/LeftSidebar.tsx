...
export default function LeftSidebar({ activeTool = 'select', showGrid = true, onToolChange }: LeftSidebarProps) {
  return (
    <aside className="w-14 flex flex-col items-center py-3 gap-1 border-r border-[rgba(201,168,76,0.15)] bg-[#0d0d0d] flex-shrink-0">
      {tools.map((tool) => {
        const isActive = tool.id === 'grid' ? showGrid : activeTool === tool.id
        return (
          <button
            key={tool.id}
            title={tool.label}
